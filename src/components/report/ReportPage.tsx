...
function inputCls(hasError?: boolean) {
  return [
    "w-full rounded-xl border bg-white dark:bg-slate-900 px-4 py-3 text-sm text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500",
...
export function ReportPage({ propertyId, propertyTitle }: Props) {
  ...
  if (submitted) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-slate-50 dark:bg-slate-950 px-4 py-20">
        <div className="w-full max-w-md text-center">
          ...
          <h1 className="mt-6 text-2xl font-extrabold text-slate-900 dark:text-white">Report Submitted</h1>
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:justify-center">
            <Link href="/explore" className="rounded-xl bg-emerald-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-emerald-700">
              Back to Explore
            </Link>
            {propertyId && (
              <Link href={`/properties/${propertyId}`} className="rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 px-5 py-3 text-sm font-semibold text-slate-700 dark:text-slate-300 transition hover:bg-slate-50 dark:hover:bg-slate-800">
                Back to detail
              </Link>
            )}
          </div>
          ...
          {propertyTitle ? (
            <div className="flex items-center gap-4">
              <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-rose-50 dark:bg-rose-900/20">
                <FlagIcon className="h-6 w-6 text-rose-500 dark:text-rose-400" />
              </div>
              <div className="min-w-0">
                <p className="text-xs font-bold uppercase tracking-widest text-rose-500 dark:text-rose-400">Reporting Property</p>
                <h1 className="mt-0.5 truncate text-2xl font-extrabold capitalize text-slate-900 dark:text-white">
            ...
            <div>
              <h1 className="text-3xl font-extrabold text-slate-900 dark:text-white">Report an Issue</h1>
        ...
        <form onSubmit={handleSubmit} noValidate>
          <div className="space-y-12">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <Field id={`${uid}-phone`} label="Phone Number" required error={errors.phone}>
                  <div className="relative">
                    <span className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-4 text-sm font-semibold text-slate-500 dark:text-slate-400">
                      +256
                    </span>
                    <input id={`${uid}-phone`} type="tel" placeholder="700 000 000" autoComplete="tel" value={phone} onChange={(e) => setPhone(e.target.value.replace(/\D/g, "").slice(0, 9))} disabled={submitting} className={[inputCls(!!errors.phone), "pl-16"].join(" ")} />
                  </div>
                </Field>
            ...
            <div className="flex justify-end">
              <button type="submit" disabled={submitting} className="w-full md:w-auto rounded-xl bg-emerald-600 px-8 py-4 text-base font-bold text-white transition hover:bg-emerald-700 disabled:opacity-60">
