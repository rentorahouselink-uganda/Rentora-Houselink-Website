...
export function SimilarProperties({ property }: Props) {
    ...
    async function load() {
      try {
        const primary = await getProperties({
          districtId: property.district.id, type: property.type, status: "AVAILABLE", limit: 6, page: 1,
        });

        const primaryList = primary.data.filter((p) => p.id !== property.id).slice(0, 5);

        if (!cancelled) {
          if (primaryList.length >= 3) {
            setProperties(primaryList); setLoading(false); return;
          }
          ...
          const fallback = await getProperties({
            type: property.type, status: "AVAILABLE", limit: needed + seenIds.size, page: 1,
          });

          const fallbackList = fallback.data.filter((p) => !seenIds.has(p.id)).slice(0, needed);
  ...
  if (properties.length === 0) return null; // If empty, we just won't show the section
  ...
  return (
    <div>
      <div className="mb-6 flex items-end justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
            More {formatLabel(property.type).toLowerCase()} listings near {property.district.name}
          </p>
        </div>
        <Link
          href={`/explore?type=${property.type}&districtId=${property.district.id}`}
          className="hidden sm:inline-flex items-center gap-1.5 text-sm font-semibold text-emerald-600 dark:text-emerald-500 transition hover:text-emerald-700 dark:hover:text-emerald-400"
        >
      ...
      <div className="mt-6 sm:hidden">
        <Link
          href={`/explore?type=${property.type}&districtId=${property.district.id}`}
          className="inline-flex items-center gap-1.5 text-sm font-semibold text-emerald-600 dark:text-emerald-500 transition hover:text-emerald-700"
        >
