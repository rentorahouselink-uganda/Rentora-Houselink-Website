        ...
        <div className="px-6 pb-6 pt-5 space-y-5">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="min-w-0">
              <p className="font-bold text-zinc-900 dark:text-white truncate">{contact.name}</p>
              <p className="text-xs text-zinc-500 dark:text-zinc-400 capitalize">{contact.role.replace(/_/g, " ").toLowerCase()}</p>
              <p className="text-sm font-medium text-zinc-600 dark:text-zinc-300">{contact.phone}</p>
            </div>
          </div>

          {/* Property reference */}
          <div className="border-l-2 border-emerald-600 dark:border-emerald-500 pl-4 py-1">
            ...
            >
              {copied ? (
                <>
                  ...
                  <span className="text-emerald-600 dark:text-emerald-500">Copied to clipboard</span>
                </>
              ) : (
                <>
                  <DocumentDuplicateIcon className="h-3.5 w-3.5" strokeWidth={1.5} />
                  Copy phone number
                </>
              )}
            </button>
